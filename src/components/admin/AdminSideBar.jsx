...
const navItems = [
  ...
  { to: 'orders', label: 'Orders', icon: <FaFlag /> },
...
const AdminSidebar = () => {
  ...
  return (
    <aside className="w-64 bg-orange-200 text-white shadow-2xl min-h-screen flex flex-col justify-between">
      <div>
        <div className="p-6 text-xl font-extrabold tracking-wide border-b border-orange-200">
          Admin Dashboard
        </div>
        <nav className="p-4 space-y-2">
          {navItems.map(({ to, label, icon }) => (
            ...
            >
              ...
              <span className="text-md text-white font-medium">{label}</span>
            </NavLink>
          ))}
        </nav>
      </div>
